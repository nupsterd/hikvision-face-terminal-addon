...
def test_emit_face_75_pasa_verify_false_paridad_forward_to_ha():
    """§5.9.510 canónico Chat 7 S10: paridad con forward_to_ha línea 494 y
    fan-out DS-K1T344 línea 636. HTTPS con cert self-signed HA banco requiere
    verify=False, sin el fix el emit falla con RemoteDisconnected empírico."""
    ...
    record = {
        ...
        "raw": {"AccessControllerEvent": {"name": "DM"}},
        "card_no": None,
        "face_rect": {"height": 0.4, "width": 0.2, "x": 0.4, "y": 0.4},
        "verify_mode": "faceOrFpOrCardOrPw",
        "device_ip": "192.168.18.202",
        "device_mac": "a4:d5:c2:75:fd:64",
        "device_ts": "2026-07-18T18:05:59-05:00",
    ...
    with patch("hikvision_face_terminal.listener.requests.post") as mock_post:
        from hikvision_face_terminal.listener import _maybe_emit_ha_webhook

        _maybe_emit_ha_webhook(
            record,
            "https://192.168.18.60:8123/api/webhook/test",
            timeout_seconds=3,
            edificio_slug="conjunto-piloto",
        ...
        assert kwargs.get("verify") is False, (
            "requests.post debe pasar verify=False para HTTPS con "
            "cert self-signed HA banco (paridad §5.9.510 Chat 7 S10)"
        )


def test_emit_fingerprint_38_hace_post():
    """§5.9.512 canónico Chat 7 S10: firmware V4.31 emite (5,38) Fingerprint Auth Passed
    empíricamente confirmado en 2 gestos. Discriminador debe incluir sub=38 para paridad
    con face (75) y card (1)."""
    from unittest.mock import patch
    ...
    record = {
        ...
        "major": 5,
        "sub": 38,
        "sub_name": "Fingerprint Auth Passed",
        "device_kind": "face_terminal",
        "employee_no": "9001",
        "raw": {"AccessControllerEvent": {"name": "DM"}},
        "card_no": "2641394168",
        ...
        "device_ip": "192.168.18.202",
        "device_mac": "a4:d5:c2:75:fd:64",
        "device_ts": "2026-07-18T20:35:00-05:00",
        "received_ts": "2026-07-18T20:35:02-05:00",
    }
    ...
    with patch("hikvision_face_terminal.listener.requests.post") as mock_post:
        ...
        assert mock_post.called, "fingerprint (5,38) debe disparar emit HA webhook"
        _, kwargs = mock_post.call_args
        assert kwargs.get("verify") is False
        payload = kwargs.get("json", {})
        assert payload.get("sub") == 38
        assert payload.get("sub_name") == "Fingerprint Auth Passed"
